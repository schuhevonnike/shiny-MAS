...
def initialize_agents(env, device):
    ...
    for agent_id in env.possible_agents:
        ...
        agents[agent_id] = DQNAgent(state_size, action_size, device=device)
    return agents

def run_experiment(env_fn, num_episodes,seed, device):
    env = env_fn()
    # Initializing agents:
    ...
    avg_training_rewards = train(agents, num_episodes=num_episodes, seed=seed, device=device)
    ...
    avg_evaluation_rewards = evaluate(agents, num_episodes=num_episodes, seed=seed,device=device)
...
def start_seed(seed):
    ...
    run_experiment(env_fn=make_env, num_episodes=args.num_episodes, seed=seed, device=device)